...
// TestGetEnvBoolWithDefault 测试布尔环境变量解析
func TestGetEnvBoolWithDefault(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "未设置时使用默认 true",
			key:          "TEST_BOOL_ENV_NOT_SET_TRUE",
			defaultValue: true,
			expected:     true,
			setEnv:       false,
		},
		{
			name:         "未设置时使用默认 false",
			key:          "TEST_BOOL_ENV_NOT_SET_FALSE",
			defaultValue: false,
			expected:     false,
			setEnv:       false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := getEnvBoolWithDefault(tt.key, tt.defaultValue)
			if result != tt.expected {
				t.Errorf("期望 %t，实际 %t", tt.expected, result)
			}
		})
	}
}
